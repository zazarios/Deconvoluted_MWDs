...
The public GitHub repository is intentionally kept compact:
...
All MATLAB packages and reproducibility blocks are distributed inside `MWDs.zip`.
Download and extract this archive before running any MATLAB entry point.
...
The six Stage-K/Pass-7-to-Pass-11 directories retain their qualified release names.
The two additional directories are explicitly post-freeze reproducibility blocks;
they do not modify or replace the qualified model chain.

## MATLAB environment

The archived MATLAB packages were developed and checked with **MATLAB R2025b**.

No internet download is required during execution of the deposited calculations. Numerical inputs needed by the archived packages are included inside `MWDs.zip` in their respective package directories.
...
No manifest-listed payload file is missing or hash-mismatched in the prepared `MWDs.zip` archive.

For reproducibility testing, extract `MWDs.zip` and run the analyses from a working copy so that the deposited SHA-256 snapshots remain unchanged.

## Quick start

Before running any analysis:

1. Download `MWDs.zip` from the repository.
2. Extract `MWDs.zip` to a local working directory.
3. Enter the relevant package directory listed below.
4. Run the corresponding MATLAB entry point.

All package names and internal paths below refer to directories inside the extracted `MWDs.zip` archive.
