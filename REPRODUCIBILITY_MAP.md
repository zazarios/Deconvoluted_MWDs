...
All package names and output paths in this document refer to directories inside the extracted `MWDs.zip` archive. The GitHub repository root contains `MWDs.zip` together with the repository-level documentation and license.
...
Before using any entry point:

1. Download `MWDs.zip`.
2. Extract the archive.
3. Enter the relevant package directory shown below.
4. Run the corresponding MATLAB entry point.

The commands and paths below are relative to the extracted archive contents.
