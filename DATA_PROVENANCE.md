...
All package names and file paths below refer to the directory structure obtained after extracting `MWDs.zip`. The GitHub repository root contains `MWDs.zip`; the qualified MATLAB packages and reproducibility blocks are stored inside that archive.
...
Inside the extracted `MWDs.zip` archive, the package
